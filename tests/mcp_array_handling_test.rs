...
fn test_generated_mcp_server_array_handling() {
    ...
    let generated_handler_path = Path::new("target/tmp/e2e-tests/e2e_mcp_server/src/handlers/find_pets_by_tags.rs");
...
fn test_openapi_spec_array_parameter_detection() {
    ...
    assert_eq!(
        array_param["schema"]["type"].as_str(),
        Some("array")
    );
